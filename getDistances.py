...
distances = {
	...
	"Macapá:Boa Vista": 1115,
	"Macapá:Brasília": 1797,
	"Macapá:Campo Grande": 2310,
	"Macapá:Cuiabá": 1820,
	"Macapá:Curitiba": 2834,
	"Macapá:Florianópolis": 3091,
	"Macapá:Fortaleza": 1457,
	"Macapá:Goiânia": 1890,
	"Teresina:Macapá": 1086,
	"Vitória:Macapá": 2544,
	"São Paulo:Macapá": 2669,
	"Salvador:Macapá": 2004,
	"São Luis:Macapá": 799,
	"Rio de Janeiro:Macapá": 2689,
	"Rio Branco:Macapá": 2180,
	"Porto Velho:Macapá": 1722,
	"Macapá:João Pessoa": 1967,
	"Recife:Macapá": 2002,
	"Porto Alegre:Macapá": 3331,
	"Palmas:Macapá": 1169,
	"Natal:Macapá": 1880,
	"Manaus:Macapá": 1037,
	"Maceió:Macapá": 2010,
	...
	"Vitória:Vitória": 0,
	"Palmas:Boa Vista": 1996
}
#popPerDis = np.zeros((27,27))
popPerDis = {}
aux = 0
for i in capitais_estaduais:
    popPerDis[i] = np.zeros(27)
    aux+=1
...
for i in distances:
    aux = i.split(":")
    if aux[0] == 'Manaus' or aux[1] == 'Manaus':
        print(index[aux[0]])
        print(index[aux[1]])
    if distances[i] == 0:
        continue
    #print(aux[0], aux[1], index[aux[0]], index[aux[1]], pops[index[aux[0]]],pops[index[aux[1]]], distances[i])
    popPerDis[index[aux[0]]][index[aux[1]]]=int(pops[index[aux[0]]])+int(pops[index[aux[1]]])/distances[i]
    popPerDis[index[aux[1]]][index[aux[0]]]=int(pops[index[aux[0]]])+int(pops[index[aux[1]]])/distances[i]
